...
/** Отметка по умолчанию: пока задание не решено, оно «не получилось» — балл растёт от нуля вверх */
...
const MARKS: { value: TaskMark; label: string; title: string }[] = [
  { value: 'ok', label: 'получилось', title: 'Решено полностью — балл' },
  { value: 'half', label: '?', title: 'Наполовину: вычислительная ошибка или решение с подсказкой — полбалла' },
  { value: 'fail', label: 'не получилось', title: 'Не решено — ноль. Так считается и то, что вы ещё не отмечали' },
]
...
/* Балл считается по всему списку: неотмеченное задание — это «не получилось», а не «не в счёт»,
   поэтому процент растёт от нуля по мере того, как задания отмечаются решёнными. */

...
export function ErrorLog({ state, onAddError, onToggleRepeat, onDeleteError }: ErrorLogProps) {
  ...
  /** Плоский список тем: названия всех занятий итогового плана без повторов */
  const topicOptions = [...new Set(planOf(state).sessions.filter((session) => session.kind !== 'rest').map((session) => session.title))]
  ...
  return (
    <section className="page-section" aria-labelledby="errors-title">
      ...
      <ErrorForm topicOptions={topicOptions} onAddError={onAddError} />
...
function ErrorForm({ topicOptions, onAddError }: ErrorFormProps) {
